...
namespace renderer
{
    Model::Model(Renderer* renderer, scene::Camera* camera)
        : mRenderer(renderer)
        , mCamera(camera)
        , mNumMesh(0)
        , mNumVertex(0)
    ...
    void Model::Draw()
    {
        ...
        constexpr uint32 stride = sizeof(renderer::Vertex);
    ...
    HRESULT Model::SetupMesh(ModelImporter& importer)
    {
        ...
        // 하나로 뭉침
        mVertices = new renderer::Vertex[sumVertexCount];
        ...
        renderer::Vertex* posInVertices = mVertices;
        ...
        desc.ByteWidth = sizeof(renderer::Vertex) * sumVertexCount;
